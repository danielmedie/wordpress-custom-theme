<?php wp_footer(); ?>
<footer id="footer">
    <div class="container">
        <div class="row top">
            <div class="col-xs-12 col-sm-6 col-md-4 footer-section about-us">
                <h4>Kort om oss</h4>
                <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed sed sodales mauris. Aliquam felis est, efficitur vel fringilla quis, vehicula quis ex.</p>
                <p>Phasellus tristique nunc in leo faucibus, a consequat nulla sagittis.</p>
            </div>

            <div class="col-xs-12 col-sm-3 col-md-3 col-md-offset-1 footer-section contact-details">
                <h4>Kontaktuppgifter</h4>
                <p>The Company</p>
                <p>Gatgatan 1</p>
                <p>123 45 Någonstans</p>
                <p>Tel: 0123456789</p>
                <p>E-post: info@thecompany.com</p>
            </div>

            <div class="col-xs-12 col-sm-3 col-md-3 col-md-offset-1 footer-section social-media">
                <h4>Social media</h4>
                <ul class="social">
                    <li>
                        <i class="fa fa-facebook"></i> <a href="#">Facebook</a>
                    </li>
                    <li>
                        <i class="fa fa-twitter"></i> <a href="#">Twitter</a>
                    </li>
                    <li>
                        <i class="fa fa-instagram"></i> <a href="#">Instagram</a>
                    </li>
                    <li>
                        <i class="fa fa-linkedin"></i> <a href="#">LinkedIn</a>
                    </li>
                </ul>
            </div>
        </div>

        <div class="row bottom">
            <div class="col-xs-12">
                <p>Copyright &copy; Grupp X, 2017</p>
            </div>
        </div>
    </div>
</footer>
